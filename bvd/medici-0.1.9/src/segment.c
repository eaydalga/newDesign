...
void
edi_segment_clear (edi_segment_t * this)
{
  int n, m;
  ...
  for (n = 0; n < EDI_NELEMS; n++)
	...
	{
	  this->defined[n][m] = 0;
	  edi_buffer_clear (&(this->elements[n][m]));
	}
}


void
edi_segment_free (edi_segment_t * this)
{
  if (!this)
    return;

  edi_segment_clear (this);
...
edi_segment_create (void)
{
  ...
  if ((s = malloc (sizeof (edi_segment_t))))
    edi_segment_init (s);
  return s;
}
...
{
  return s->defined[x][y] ? edi_buffer_data (&(s->elements[x][y])) : NULL;
}
...
edi_segment_copy (edi_segment_t * dst, edi_segment_t * src)
{
  ...
  *dst = *src;
  ...
  for (x = 0; x < EDI_NELEMS; x++)
	...
	{
	  dst->defined[x][y] = 0;
	  edi_segment_set_element (dst, x, y,
