...
/* 'THIS' is the "parent class" of 'this' */
#define this ((edifact_t *)THIS)

#define MSGDIR THIS->message
#define SVCDIR THIS->service
...
static int
edifact_is_in_charset (edi_parser_t * THIS, unsigned char c)
{
  switch (this->syntax_level)
    {
    case ASCII_A:
    ...
    case ASCII_B:
      if (!((c == ISO9735_IS4 || c == ISO9735_IS3 || c == ISO9735_IS1) ||
	    ...
	    (c >= ASCII_PERCENT && c <= ASCII_QUESTIONMARK) ||
	    (c >= ASCII_A && c <= ASCII_Z) || (c >= ASCII_a && c <= ASCII_z)))
	return 0;
      break;
...
static void edifact_set_parameters
  (edi_parser_t * THIS, edi_segment_t * segment,
  ...
  switch (edifact_get_segment_code (segment))
    ...
    case UNB:
      set_params (parameters, SVCDIR, segment,
		  ...
		  TestIndicator, "0035", NULL, LastParameter);
    ...
    case UNG:
      set_params (parameters, SVCDIR, segment,
		  ...
		  ApplicationPassword, "0058", NULL, LastParameter);
      break;
    ...
    case UNH:
      set_params (parameters, SVCDIR, segment,
		  ...
		  FirstAndLastTransfer, "S010", "0073", LastParameter);
      break;
    ...
    case UNT:
      set_params (parameters, SVCDIR, segment,
		  ...
		  MessageReferenceNumber, "0062", NULL, LastParameter);
      break;

    case UNZ:
      set_params (parameters, SVCDIR, segment,
		  InterchangeControlCount, "0036", NULL,
		  InterchangeControlReference, "0020", NULL, LastParameter);
      break;

    case UNS:
      set_params (parameters, SVCDIR, segment,
		  SectionId, "0081", NULL, LastParameter);
      break;

    default:
      set_params (parameters, SVCDIR, segment, LastParameter);
    }
}


static edi_error_t
edifact_segment (edi_parser_t * THIS)
{
  edi_parameters_t parameters;
  edi_segment_t *segment, *context;
  edifact_code_t segment_code, context_code;
  char *transaction, *tmp, *str1, *str2;

  segment = THIS->segment;
  context = edi_parser_peek_segment (THIS);
  segment_code = edifact_get_segment_code (segment);
  context_code = edifact_get_segment_code (context);

  edifact_set_parameters (THIS, segment, &parameters);
  ...
  /*
     ...
     edi_parser_raise_error (THIS, EDI_ESEGMENT);
   */
  ...
  switch (segment_code)
    ...
    case UNB:
      if (context_code)
	return edi_parser_raise_error (THIS, EDI_EENVELOPE);
      edi_parser_push_segment (THIS, segment);
      this->message_count = 0;
      this->segment_count = 0;
      this->group_count = 0;
      /* level = "(//UNB/S001/0001)[4]", version = "//UNB/S001/0002" */
      /* FIXME - need to set character set in the lexer fsa */
      if ((tmp = edi_segment_get_element (segment, 0, 0)))
	this->syntax_level = tmp[3];
      if ((tmp = edi_segment_get_element (segment, 0, 1)))
	this->syntax_version = tmp[0];
      /* FIXME - switch() on syntax version  */
      edi_parser_handle_start (THIS, EDI_INTERCHANGE, &parameters);
      edi_parser_handle_segment (THIS, &parameters, SVCDIR);
      break;
	...
	return edi_parser_raise_error (THIS, EDI_EENVELOPE);
      ...
      this->group_count++;
      this->message_count = 0;
      edi_parser_handle_start (THIS, EDI_GROUP, &parameters);
      edi_parser_handle_segment (THIS, &parameters, SVCDIR);
      break;
	...
	return edi_parser_raise_error (THIS, EDI_EENVELOPE);
	  ...
	  && strcmp (str1, str2))
	return edi_parser_raise_error (THIS, EDI_EGMT);
      /* if in a functional group, only one message version is allowed */
      if (context_code == UNG &&
	  (str1 = edi_get_element_by_name (SVCDIR, context, "S008", "0052"))
	  && (str2 =
	      edi_get_element_by_name (SVCDIR, segment, "S009", "0052"))
	  && strcmp (str1, str2))
	return edi_parser_raise_error (THIS, EDI_EGMV);
      edi_parser_push_segment (THIS, segment);
      this->message_count++;
      this->segment_count = 1;	/* must include UNH (and UNT) */
      edi_parser_handle_start (THIS, EDI_TRANSACTION, &parameters);
      transaction = edi_segment_get_element (segment, 1, 0);
      MSGDIR = edi_parser_handle_directory (THIS, &parameters);
      edi_directory_head (MSGDIR, segment, &parameters,
			  THIS, SVCDIR, transaction);
      break;
    ...
    case UNT:
      ...
      if (context_code != UNH)
	return edi_parser_raise_error (THIS, EDI_EENVELOPE);
      if (strcmp (edi_segment_get_element (segment, 1, 0),
		  edi_segment_get_element (context, 0, 0)))
	return edi_parser_raise_error (THIS, EDI_ETTR);
      if (atoi (edi_segment_get_element (segment, 0, 0)) !=
	  this->segment_count)
	return edi_parser_raise_error (THIS, EDI_ETTC);
      edi_parser_pop_segment (THIS);
      edi_directory_tail (MSGDIR, segment, &parameters, THIS, SVCDIR);
      MSGDIR = NULL;
      edi_parser_handle_end (THIS, EDI_TRANSACTION, NULL);
      this->segment_count++;
      break;
	...
	return edi_parser_raise_error (THIS, EDI_EGEMPTY);
      if (strcmp (edi_segment_get_element (segment, 1, 0),
		  edi_segment_get_element (context, 4, 0)))
	return edi_parser_raise_error (THIS, EDI_EGTR);
      if (atoi (edi_segment_get_element (segment, 0, 0)) !=
	  this->message_count)
	return edi_parser_raise_error (THIS, EDI_EGTC);
      edi_parser_pop_segment (THIS);
      edi_parser_handle_segment (THIS, &parameters, SVCDIR);
      edi_parser_handle_end (THIS, EDI_GROUP, NULL);
      break;
    ...
    case UNZ:
      if (context_code != UNB)
	return edi_parser_raise_error (THIS, EDI_EENVELOPE);
      if (!(this->group_count || this->message_count))
	return edi_parser_raise_error (THIS, EDI_EIEMPTY);
      if (strcmp (edi_segment_get_element (segment, 1, 0),
		  edi_segment_get_element (context, 4, 0)))
	return edi_parser_raise_error (THIS, EDI_EITR);
      if (atoi (edi_segment_get_element (segment, 0, 0)) !=
	  (this->group_count ? this->group_count : this->message_count))
	return edi_parser_raise_error (THIS, EDI_EITC);
      edi_parser_pop_segment (THIS);
      edi_parser_handle_segment (THIS, &parameters, SVCDIR);
      edi_parser_handle_end (THIS, EDI_INTERCHANGE, NULL);
      break;
    ...
    case UNS:
      ...
      if (context_code != UNH)
	return edi_parser_raise_error (THIS, EDI_EENVELOPE);
      edi_directory_body (MSGDIR, segment, &parameters, THIS, SVCDIR);
      /* FIXME - should handling sections be the job of the tsg? */
      /* edi_parser_handle_start (THIS, EDI_SECTION, &parameters); */
      /* edi_parser_handle_end (THIS, EDI_SECTION, NULL); */
      break;
    ...
    default:
      ...
      if (context_code != UNH)
	return edi_parser_raise_error (THIS, EDI_EENVELOPE);
      edi_directory_body (MSGDIR, segment, &parameters, THIS, NULL);
      break;
    }
...
static void
edifact_fini (edi_parser_t * THIS)
{
  if (MSGDIR)
...
edi_error_t
edi_edifact_init (edi_parser_t * THIS)
{
  edi_advice_t *advice = &(THIS->tokeniser.advice);

  /* mitigate uninitialised variables */
  memset (THIS, 0, sizeof (edifact_t));

  /* set all dynamic pointers to NULL */
  SVCDIR = NULL;
  MSGDIR = NULL;

  /* initalise service segment parser */
  SVCDIR = EDIFACT_UNO ();
  this->syntax_version = ASCII_2;	/* FIXME */
  this->syntax_level = ASCII_A;	/* FIXME */

  /* morph parser object to use this syntax */
  THIS->syntax_fini = edifact_fini;
  THIS->sgmnt_handler = edifact_segment;

  /* set "service string advice" to edifact defaults */
  edi_advice_set_ss (advice, 1, ASCII_COLON);
  edi_advice_set_es (advice, 1, ASCII_PLUS);
  edi_advice_set_ts (advice, 1, ASCII_PLUS);
  edi_advice_set_dn (advice, 1, ASCII_COMMA);
  edi_advice_set_ri (advice, 1, ASCII_QUESTIONMARK);
  edi_advice_set_st (advice, 1, ASCII_APOSTROPHE);
...
static edi_francesco_element_info_t elementinfo[] = {
  {"0001", EDI_ISO2382A, 4, 4, "Syntax identifier",
   "a3 upper case Controlling Agency (e.g. UNO=UN/ECE) and a1 stating level (e.g. A)  (which together give UNOA)"},

#if EDI_UNO_VERSION == 1
  {"0002", EDI_ISO2382N, 1, 1, "Syntax version number",
   "Increments 1 for each new version. Shall be 1 to indicate this version"},
#else
  {"0002", EDI_ISO2382N, 1, 1, "Syntax version number",
   "Increments 1 for each new version. Shall be 2 to indicate this version"},
#endif

  {"0004", EDI_ISO2382X, 1, 35, "Sender identification",
   "Code or name as specified in IA"},
  {"0007", EDI_ISO2382X, 1, 4, "Partner identification code qualifier",
   "Used with sender/recipient identification code"},
  {"0008", EDI_ISO2382X, 1, 14, "Address for reverse routing",
   "Address for reverse routing"},
  {"0010", EDI_ISO2382X, 1, 35, "Recipient Identification",
   "Code or name as specified in IA"},
  {"0014", EDI_ISO2382X, 1, 14, "Routing address",
  ...
  {"0022", EDI_ISO2382X, 1, 14, "Recipient's reference/password",
   "As specified in IA. May be password to recipient's system or to third party network"},
  {"0025", EDI_ISO2382X, 2, 2, "Recipient's reference/password qualifier",
   "If specified in IA"},
  ...
  {"0029", EDI_ISO2382A, 1, 1, "PROCESSING PRIORITY CODE",
   "Used if specified in IA"},
  {"0031", EDI_ISO2382N, 1, 1, "ACKNOWLEDGEMENT REQUEST",
   "Set = 1 if sender requests acknowledgement, i.e. UNB and UNZ segments received and identified"},
  {"0032", EDI_ISO2382X, 1, 35, "COMMUNICATIONS AGREEMENT ID",
  ...
  {"0035", EDI_ISO2382N, 1, 1, "TEST INDICATOR",
   "Set = 1 if the interchange is a test. Otherwise not used"},
  {"0036", EDI_ISO2382N, 1, 6, "INTERCHANGE CONTROL COUNT",
   "The count of the number of messages or, if used, the number of functional groups in the interchange. One of these counts shall appear."},
  {"0038", EDI_ISO2382X, 1, 6, "FUNCTIONAL GROUP IDENTIFICATION",
   "Identifies the one message type in the functional group"},
  {"0040", EDI_ISO2382X, 1, 35, "Application sender's identification",
   "Code or name identifying the division, department etc. within the originating sender's organization"},
  {"0044", EDI_ISO2382X, 1, 35, "Recipient's identification",
   "Code or name identifying the division,department etc. within the recipients organization for which the group of messages is intended"},
  {"0048", EDI_ISO2382X, 1, 14, "FUNCTIONAL GROUP REFERENCE NUMBER",
   "Unique reference number assigned by sender's division, department etc."},
  {"0051", EDI_ISO2382X, 1, 2, "CONTROLLING AGENCY",
   "Code to identify the agency controlling the specification, maintenance and publication of the message type"},
  {"0052", EDI_ISO2382X, 1, 3, "Message version number",
   "Version number of the message type in the functional group"},
  {"0054", EDI_ISO2382X, 1, 3, "Message release number",
   "Release number within current version number"},
  {"0057", EDI_ISO2382X, 1, 6, "Association assigned Code",
   "A code assigned by the association responsible for the design and maintenance of the type of message concerned"},
  {"0058", EDI_ISO2382X, 1, 14, "APPLICATION PASSWORD",
   "Password to recepient's division, department or sectional system (if required)"},
  {"0060", EDI_ISO2382N, 1, 6, "NUMBER OF MESSAGES",
   "The count of the number of messages in the functional group"},
  {"0062", EDI_ISO2382X, 1, 14, "MESSAGE REFERENCE NUMBER",
   "A sender's unique message reference"},
  {"0065", EDI_ISO2382X, 1, 6, "Message type",
   "Type of message being transmitted"},
  {"0068", EDI_ISO2382X, 1, 35, "COMMON ACCESS REFERENCE",
   "Key to relate all subsequent transfers of data to the same business case of file. Within the 35 characters the IA may specify component elements"},
  {"0070", EDI_ISO2382N, 1, 2, "Sequence of transfers",
   "Starts at 1 and is incremented by 1 for each transfer"},
  {"0073", EDI_ISO2382A, 1, 1, "First and last transfer",
   "C = Creation, must be present for first transfer if more than one foreseen, F = Final, must be present for last transfer"},
  {"0074", EDI_ISO2382N, 1, 6, "NUMBER OF SEGMENTS IN THE MESSAGE",
   "Control count including UNH and UNT"},
  {"0077", EDI_ISO2382X, 3, 3, "TEXT REFERENCE CODE",
   "Qualifies and identifies the purpose and function of the segment if indicated in the message specification"},
  {"0078", EDI_ISO2382X, 1, 70, "FREE FORM TEXT",
   "Not machine-processable information"},
  {"0081", EDI_ISO2382A, 1, 1, "SECTION IDENTIFICATION",
   "Separates sections in a message by one of the following codes: D separates the header and detail sections S separates the detail and summary sections"},
  {NULL}
};
...
static edi_francesco_cmpsite_info_t compositeinfo[] = {
  ...
  {"S005", "RECIPIENTS REFERENCE", ""},
  {"S006", "APPLICATION SENDER'S IDENTIFICATION", ""},
  {"S007", "APPLICATION RECIPIENTS IDENTIFICATION", ""},
...
static edi_francesco_segment_info_t segmentinfo[] = {
  ...
  {"TXT", "Text",
   "To give information in addition to that in other segments in the service message, as required"},
  {"UNS", "Section Control",
...
static edi_francesco_codelst_info_t codelistinfo[] = {
  ...
  {"0001", "UNOC", "UN/ECE level C",
   "As defined in ISO 8859-1 : Information processing - Part 1: Latin alphabet No. 1."},
  {"0001", "UNOD", "UN/ECE level D",
   "As defined in ISO 8859-2 : Information processing - Part 2: Latin alphabet No. 2."},
  {"0001", "UNOE", "UN/ECE level E",
   "As defined in ISO 8859-5 : Information processing - Part 5: Latin/Cyrillic alphabet."},
  {"0001", "UNOF", "UN/ECE level F",
   "As defined in ISO 8859-7 : Information processing - Part 7: Latin/Greek alphabet."},
  {"0007", "1", "DUNS (Dun + Bradstreet)", "Self explanatory."},
  {"0007", "4", "IATA (International Air Transport Association)",
   "Self explanatory."},
  {"0007", "5", "INSEE (Institut National de la Statistique et des Etudes",
   "Economiques) - SIRET French national statistics institute. SIRET means Systeme Informatique du Repertoire des entreprises et de leurs ETablissements."},
  {"0007", "8", "UCC Communications ID (Uniform Code Council Communications",
   "Identifier) The Uniform Code Council Communications Identifier is a ten digit code used to uniquely identify physical and logical locations."},
  {"0007", "9", "DUNS (Dun + Bradstreet) with 4 digit suffix",
   "Self explanatory."},
  {"0007", "12", "Telephone number", "Self explanatory."},
  {"0007", "14", "EAN (European Article Numbering Association)",
   "Self explanatory."},
  {"0007", "18", "AIAG (Automotive Industry Action Group)",
   "Self explanatory."},
  {"0007", "22", "INSEE (Institut National de la Statistique et des Etudes",
   "Economiques) - SIREN French national statistics institute. SIREN means Systeme Informatique du Repertoire des ENtreprises (et de leurs etablissements)."},
  {"0007", "30", "ISO 6523: Organization identification",
   "Self explanatory."},
  {"0007", "31", "DIN (Deutsches Institut fuer Normung)",
   "German standardization institute."},
  {"0007", "33", "BfA (Bundesversicherungsanstalt fuer Angestellte)",
   "German social security association."},
  {"0007", "34", "National Statistical Agency", "Self explanatory."},
  {"0007", "51", "GEIS (General Electric Information Services)",
   "Self explanatory."},
  {"0007", "52", "INS (IBM Network Services)", "Self explanatory."},
  {"0007", "53", "Datenzentrale des Einzelhandels",
   "German data centre for retail trade."},
  {"0007", "54", "Bundesverband der Deutschen Baustoffhaendler",
   "German building material trade association."},
  {"0007", "55", "Bank identifier code", "Self explanatory."},
  {"0007", "56", "Statens Teleforvaltning",
  ...
  {"0013", "UCS", "Segment error indication",
  ...
  {"0013", "UNA", "Service string advice",
   "To define the characters selected for use as delimiters and indicators in the rest of the interchange that follows."},
  {"0013", "UNB", "Interchange header",
   "To start, identify and specify an interchange."},
  {"0013", "UNE", "Functional group trailer",
   "To end and check the completeness of a functional group."},
  {"0013", "UNG", "Functional group header",
   "To head, identify and specify a functional group."},
  {"0013", "UNH", "Message header",
   "To head, identify and specify a message."},
  {"0013", "UNS", "Section control",
   "To separate header, detail and summary sections of a message."},
  {"0013", "UNT", "Message trailer",
   "To end and check the completeness of a message."},
  {"0013", "UNZ", "Interchange trailer",
   "To end and check the completeness of an interchange."},
  ...
  {"0051", "AB", "DIN (Deutsches Institut fuer Normung)",
   "German standardization institute."},
  {"0051", "AC", "ICS (International Chamber of Shipping)",
   "Self explanatory."},
  {"0051", "AD", "UPU (Union Postale Universelle)",
   "Universal Postal Union."},
  {"0051", "AE", "United Kingdom ANA (Article Numbering Association)",
  ...
  {"0051", "CE", "CEFIC (Conseil Europeen des Federations de l'Industrie",
   "Chimique) EDI project for chemical industry."},
  {"0051", "EC", "EDICON", "UK Construction project."},
  {"0051", "ED", "EDIFICE (Electronic industries project)",
   "EDI Forum for companies with Interest in Computing and Electronics (EDI project for EDP/ADP sector)."},
  {"0051", "EE", "EC + EFTA (European Communities and European Free Trade",
   "Association) Self explanatory."},
  {"0051", "EN", "EAN (European Article Numbering Association)",
   "Self explanatory."},
  {"0051", "ER", "UIC (International Union of railways)",
   "European railways."},
  {"0051", "EU", "European Union", "Self explanatory."},
  {"0051", "EX", "IECC (International Express Carriers Conference)",
   "Self explanatory."},
  {"0051", "IA", "IATA (International Air Transport Association)",
   "Self explanatory."},
  ...
  {"0051", "OD",
   "ODETTE (Organization for Data Exchange through Tele-Transmission in Europe)",
   "European automotive industry project."},
  {"0051", "RI", "RINET (Reinsurance and Insurance Network)",
   "Self explanatory."},
  {"0051", "RT", "UN/ECE/TRADE/WP.4/GE.1/EDIFACT Rapporteurs' Teams",
   "Self explanatory."},
  {"0051", "UN", "UN/ECE/TRADE/WP.4",
  ...
  {"0052", "D", "Draft version/UN/EDIFACT Directory",
  ...
  {"0054", "912", "Standard release 1991",
  ...
  {"0054", "932", "Standard release 1993",
  ...
  {"0065", "BAPLTE", "Bayplan/stowage plan total numbers message",
   "A code to identify the bayplan/stowage plan total numbers message."},
  {"0065", "BMISRM", "Bulk marine inspection summary report message",
  ...
  {"0065", "BOPCUS", "Balance of payment customer transaction report message",
   "A code to identify the balance of payment customer transaction report message."},
  {"0065", "BOPDIR", "Direct balance of payment declaration message",
   "A code to identify the direct balance of payment declaration message."},
  {"0065", "BOPINF", "Balance of payment information from customer message",
   "A code to identify the balance of payment information from customer message."},
  {"0065", "CALINF", "Vessel call information message",
   "A code to identify the vessel call information message."},
  {"0065", "CASINT",
   "Request for legal administration action in civil proceedings",
   "message A code to identify the request for legal administration action in civil proceedings message."},
  {"0065", "CASRES",
  ...
  {"0065", "DOCAPP", "Documentary credit application message",
  ...
  {"0081", "D", "Header/detail section separation",
   "To qualify the segment UNS, when separating the header from the detail section of a message."},
  {"0081", "S", "Detail/summary section separation",
   "To qualify the segment UNS, when separating the detail from the summary section of a message."},
  {"0083", "1", "Acknowledged (this level and all lower levels)",
   "Self explanatory."},
  {"0083", "2", "Acknowledged - errors detected and reported",
   "Self explanatory."},
  {"0083", "3", "One or more rejected - next lower level",
   "Self explanatory."},
  {"0083", "4", "This level and all lower levels rejected",
  ...
  {"0083", "7", "This level acknowledged, next lower level acknowledged if",
   "not explicitly rejected The corresponding referenced-level is acknowledged. All messages or functional groups at the next lower referenced-level are acknowledged except those explicitly reported as rejected at the next lower reporting-level in this CONTRL message."},
  {"0083", "8", "Interchange received",
  ...
  {"0085", "14", "Value not supported in this position",
  ...
  {"0085", "15", "Not supported in this position",
  ...
  {"0085", "22", "Invalid service character(s)",
  ...
  {"0085", "23", "Unknown Interchange sender",
  ...
  {"0085", "33", "Invalid occurrence outside message or functional group",
  ...
  {"0085", "38", "Missing digit in front of decimal sign",
   "Notification that a decimal sign is not preceded by one or more digits."},
  {"0085", "39", "Data element too long",
  ...
  {"0085", "40", "Data element too short",
  ...
  {"0085", "41", "Permanent communication network error",
  ...
  {"0085", "42", "Temporary communication network error",
  ...
  {"0085", "43", "Unknown interchange recipient",
   "Notification that the interchange recipient is not known by a network provider."},
  {"0085", "44", "Application temporarily unavailable",
  ...
  {"0007", "54", "Bundesverband der Deutschen Baustoffhaendler, Germany",
   NULL},
  ...
  {"0007", "90",
   "SITA (Societe Internationale de Telecommunications Aeronautiques)", NULL},
  ...
  {"0051", "CE",
   "CEFIC (Conseil Europeen des Federations de l'Industrie Chimique)", NULL},
  {"0051", "EC", "EDICON", NULL},
  ...
  {"0051", "UN", "UN/ECE/TRADE/WP.4, United Nations Standard Messages (UNSM)",
   NULL},
  ...
  {"0065", "BAPLIE",
   "Bayplan/stowage plan occupied and empty locations message", NULL},
  {"0065", "BAPLTE", "Bayplan/stowage plan total numbers message", NULL},
  {"0065", "BOPBNK",
   "Bank transactions and portfolio transactions report message", NULL},
  {"0065", "BOPCUS", "Balance of payment customer transaction report message",
   NULL},
  ...
  {"0065", "DOCAMI", "Documentary credit amendment information message",
   NULL},
  {"0065", "DOCAMR",
   "Request for an amendment of a documentary credit message", NULL},
  {"0065", "DOCAPP", "Documentary credit application message", NULL},
  {"0065", "DOCARE",
   "Response to an amendment of a documentary credit message", NULL},
  ...
  {"0065", "IFCSUM",
   "International forwarding and consolidation summary message", NULL},
  {"0065", "IFTCCA",
   "Forwarding and transport shipment charge calculation message", NULL},
  {"0065", "IFTDGN", "Dangerous goods notification message", NULL},
  {"0065", "IFTFCC",
   "International transport freight costs and other charges message", NULL},
