   ...
   edi_parameters_t * parameters)
{
  switch (ungtdi_get_segment_code (segment))
    ...
    case STX:
      set_params (parameters, SERVICE, segment, SyntaxIdentifier, "STDS", "STDS01", SyntaxVersionNumber, "STDS", "STDS02", SendersId, "FROM", "FROM01", SendersName, "FROM", "FROM02", RecipientsId, "UNTO", "UNTO01", RecipientsName, "UNTO", "UNTO02", Date, "TRDT", "TRDT01", Time, "TRDT", "TRDT02", InterchangeControlReference, "SNRF", "NULL", SendersReference, "SNRF", "NULL",	/* FIXME */
		  RecipientsReference, "RCRF", "NULL",
		  ApplicationReference, "APRF", "NULL",
		  ProcessingPriorityCode, "PRCD", "NULL", LastParameter);
    ...
    case MHD:
      set_params (parameters, SERVICE, segment,
		  ...
		  MessageVersionNumber, "TYPE", "TYPE02", LastParameter);
      break;
    ...
    case END:
      set_params (parameters, SERVICE, segment,
		  NumberOfMessages, "NMST", "NULL", LastParameter);
      break;
...
static edi_error_t
edi_ungtdi_segment (edi_parser_t * THIS)
{
  edi_parameters_t parameters;
  ...
  ungtdi_set_parameters (THIS, segment, &parameters);
  ...
  /*if (!EDI_isSegmentValid (MESSAGE && !segment_code ?
     MESSAGE : SERVICE,
     segment))
     EDIParser_raiseError (THIS, EDI_ESEGMENT); */
...
static void
ungtdi_fini (edi_parser_t * THIS)
{
}


edi_error_t
edi_ungtdi_init (edi_parser_t * THIS)
{
  edi_advice_t *advice = &(THIS->tokeniser.advice);

  memset (this, 0, sizeof (ungtdi_t));	/* mitigate bugs */

  this->done = 0;
  MESSAGE = NULL;
  SERVICE = UNGTDI_1_ANA ();

  THIS->syntax_fini = ungtdi_fini;
  THIS->sgmnt_handler = edi_ungtdi_segment;

  /* initialise lexical state automata */
  edi_advice_set_ss (advice, 1, ASCII_COLON);
  edi_advice_set_es (advice, 1, ASCII_PLUS);
  edi_advice_set_ts (advice, 1, ASCII_EQUALS);
...
static edi_francesco_element_info_t elementinfo[] = {
  {"STDS01", EDI_ISO2382X, 1, 4, "Identifier", NULL},
  {"STDS02", EDI_ISO2382X, 1, 4, "Identifier", NULL},
  {"FROM01", EDI_ISO2382X, 1, 14, "Transmission Sender", NULL},
  {"FROM02", EDI_ISO2382X, 1, 35, "Code Name", NULL},
  {"UNTO01", EDI_ISO2382X, 1, 14, "Transmission Recipient", NULL},
  {"UNTO02", EDI_ISO2382X, 1, 35, "Code Name", NULL},
  {"TRDT01", EDI_ISO2382N, 1, 6, "Transmission", NULL},
  {"TRDT02", EDI_ISO2382N, 1, 6, "Date Time", NULL},
  ...
  {"TYPE01", EDI_ISO2382X, 1, 6, "Type", NULL},
  {"TYPE02", EDI_ISO2382N, 1, 1, "Version Number", NULL},
...
static edi_francesco_cmpsite_info_t compositeinfo[] = {
  ...
  {"FROM", "Identification of", NULL},
  {"UNTO", "Identification of", NULL},
