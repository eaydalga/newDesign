...
EDI_Directory read_txttsg (char *);
...
main (int argc, char **argv)
{
  EDI_Parser parser;
  FILE *stream = stdin;
  char buffer[4096], *xmltsg = NULL, *txttsg = NULL;
  int length, done, optind, n;
  MyData mydata;
  ...
  /* If we were given an txttsg argument read it in */

  if (txttsg)
    mydata.directory = read_txttsg (txttsg);

  /* If we were given an xmltsg argument read it in */

#ifdef HAVE_LIBEXPAT
  if (xmltsg)
    mydata.directory = read_xmltsg (xmltsg);
#endif
...
void
errorhandler (void *v, int i)
{
  MyData *mydata = (MyData *) v;

  fprintf (stderr, "*** Error: Segment %ld: Character %ld: %s\n",
	   EDI_GetCurrentSegmentIndex (mydata->parser),
	   EDI_GetCurrentByteIndex (mydata->parser), EDI_GetErrorString (i));
}


void
warninghandler (void *v, int i)
{
  MyData *mydata = (MyData *) v;
...
void
starthandler (void *v, edi_event_t event, EDI_Parameters p)
{
  MyData *mydata = (MyData *) v;
  EDI_Parameter n;
  char *s;
  ...
  printf ("[ start-of-%s", edi_event_string (event));
...
void endhandler (void *v, edi_event_t event, EDI_Parameters p)
{
  MyData *mydata = (MyData *) v;
  EDI_Parameter n;
  char *s;
  ...
  printf ("[ end-of-%s", edi_event_string (event));
...
void segmenthandler
  (void *v, EDI_Parameters parameters, EDI_Segment segment,
   EDI_Directory directory)
{
  MyData *mydata = (MyData *) v;
  int n, m;
  ...
  /*directory = edi_parser_service(mydata->parser); */
  ...
  indentoutput (mydata->indent);
  printf ("| %s%-3s [%ld]",
	  EDI_GetParameter (parameters, ServiceSegment) ? "*" : "",
	  code ? code : "????",
	  ...
	  if (element)
	    {
	      ...
	      printf ("| %-6.6s    %-35.35s  ( ------ %c ) %s",
		      element, "", item.reqr ? 'M' : 'C', name ? name : "");
...
usage (void)
{
  printf
    ("\n"
     "Usage: describe [-h] [-n] [-x <xmlfile>] [-t <txtfile>] [<edifile>]\n"
     "       -h this text\n"
     "       -n don't evaluate numeric or coded elements\n"
     "       -x read directory definition from <xmlfile>\n"
     "       -t read directory definition from <txtfile>\n"
     "\n"
