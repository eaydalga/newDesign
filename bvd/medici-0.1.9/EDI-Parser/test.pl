...
sub handle_directory {
    ...
    #my($key, $val);
    #print "$key, $val\n" while(($key, $val) = each(%parameters));

    if(defined $ENV{EDI_TSG}) {
	$file = $ENV{EDI_TSG};

    } elsif($parser->type == UNGTDI) {
	$file = "$ENV{HOME}/tsg/tradacom.xml";
	
    } elsif($parser->type == X12) {
	$file = sprintf "%s/tsg/%s_%s.xml",
	$ENV{HOME},
...
sub handle_token {
    my($parser, $token) = @_;
    printf "%s", " "x$indent if $token->type == 6;
    printf "[%s]", $token->data if $token->type == 0 || $token->type == 6;
    printf "%s", $token->data if $token->type == 2 || $token->type == 3 ||
	$token->type == 4;
    printf "\n" if $token->type == 5;
}
